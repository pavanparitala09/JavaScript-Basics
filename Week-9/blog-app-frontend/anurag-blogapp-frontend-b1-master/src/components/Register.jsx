...
function Register() {
  ...
  const onUserRegister = async (newUser) => {
    ...
    Object.keys(userObj).forEach((key) => {
      payload.append(key, userObj[key]);
    });
    // add profilePic to Formdata object
    payload.append("profileImageUrl", profileImageUrl[0]);
    //add image to formData objecte
    try {
      if (role === "user") {
        //make API req to user-api
        let resObj = await axios.post("http://localhost:4000/user-api/register", payload);
      ...
      if (role === "author") {
        ...
        //make API req to user-api
        let resObj = await axios.post("http://localhost:4000/author-api/users", payload);
