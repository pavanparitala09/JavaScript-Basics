...
function Products() {
  ...
  const users = [
    { id: 1, name: "Pavan", email: "pavan@gmail.com", role: "Admin" },
    { id: 2, name: "Rahul", email: "rahul@gmail.com", role: "Editor" },
    { id: 3, name: "Anita", email: "anita@gmail.com", role: "Viewer" },
  ];
  return (
    <div>
      <h1>User Are</h1>
      {/* {products.map((proObj) => (
        <Product key={proObj.productId} product={proObj} />
      ))} */}

      {users.map((userObj) => (
        <Product key={userObj.id} user={userObj} />
      ))}
    </div>
  );
}
